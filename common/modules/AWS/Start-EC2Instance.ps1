...
$t = $i | Where-Object {$_.RunningInstance.state.Name -eq 'stopped'} | select -ExpandProperty RunningInstance | select -ExpandProperty Tags
...
{ 
    ...
    if ($list.Count -gt 0) 
    {
        ...
        $result = $InstanceID | Start-EC2Instance -Confirm -Verbose
        if ($result)
        {
